...
# 渲染题目
def render_question(question, q_type, q_index, chapter, saved_answers):
    col1, col2 = st.columns([9, 1])
    with col1:
        st.markdown(f"**{q_type} {q_index+1}.** {question['question']}")
    
    # ========== 新增：选择题选项交互 ==========
    user_selected = None  # 用户选择的答案
    is_choice_question = q_type in ["单选题", "多选题"]
    
    if is_choice_question and "options" in question:
        # 构建选项字典，用于显示
        options_dict = {}
        for opt in question["options"]:
            # 提取选项字母和内容，如 "A. xxx" -> {"A": "xxx"}
            if ". " in opt:
                letter = opt.split(". ")[0]
                content = opt.split(". ", 1)[1] if ". " in opt else opt
                options_dict[letter] = opt
            else:
                # 如果没有标准格式，直接使用完整选项
                options_dict[opt] = opt
        
        # 获取当前题目的 session_state key
        state_key = f"choice_{chapter}_{q_type}_{question['id']}"
        ...
        if q_type == "单选题":
            ...
            if selected:
                ...
                # 标记该题已作答
                st.session_state[f"answered_{chapter}_{q_type}_{question['id']}"] = True
        else:
            # 多选题：使用 multiselect 或 checkbox
            ...
            if selected:
                user_selected = "".join(sorted(selected))  # 组合成如 "ABC"
                st.session_state[f"answered_{chapter}_{q_type}_{question['id']}"] = True
    ...
    # ========== 答案折叠区域（修改：根据是否已作答自动展开） ==========
    # 判断是否已作答
    answered_key = f"answered_{chapter}_{q_type}_{question['id']}"
    has_answered = st.session_state.get(answered_key, False)
    
    # 如果是选择题且已作答，默认展开；否则保持折叠
